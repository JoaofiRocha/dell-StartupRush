...
public class Routes {
    ...
    //add event
    @PostMapping("/manageBattle/{startupId}/setEvent")
    public StartupEntity getStartupSetEvent(@PathVariable("startupId") int startupId, @RequestBody List<String> events){
        ...
        startup.setEvents(events.stream()
                .map(Events::valueOf)
                .collect(Collectors.toList()));
        return startup;
    }
    ...
    public int nextRound(){
        return nextRoundUseCase.execute();
    }
    ...
    @GetMapping("/competitionWinner")
    public StartupEntity getWinner(){
